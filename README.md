...
**Brew formulae:** `starship eza bat fzf zoxide fd git-delta lazygit lazydocker` (Zsh ecosystem), `jq yq htop tree btop herdr glow bandwhich dust hunk tflint` (CLI utilities), `gh glab` (Git platform CLIs), `azure-cli` (Cloud platform CLIs), `kubectl helm k9s` (Kubernetes), `cmake` (build), `neovim node npm ripgrep` (AstroNvim prerequisites).
